...
function Quiz() {
  ...
  const quizQuestions = [
    ...
    {
      id: "q2",
      question: "Your mood today?",
      options: ["Chill", "Adventurous", "Cozy"]
    },
    {
      id: "q3",
      question: "Spice preference?",
      options: ["Mild", "Kick", "Fiery"]
    },
    {
      id: "q4",
      question: "Meal craving?",
      options: ["Snack Attack", "Big-Bite"]
    },
    {
      id: "q5",
      question: "Favorite flavor?",
      options: ["Sweet", "Savory", "Spicy", "Cheesy"]
    },
    {
      id: "q6",
      question: "Choose your flavor delivery method",
      options: ["Baked & Light", "Roasted & Flavorful", "Grilled & Bold", "Fried & Crispy"]
    }
  ];
  ...
  const handleSubmit = (e) => {
    ...
    console.log("Submitted Tags:", answersList);
  };
  ...
  const getRecommendedItems = () => {
    ...
    let filtered = menuItems.filter(item =>
      item.recommendation_tags.includes(basePreference)
    );

    filtered.sort((a, b) => {
      const aMatches = a.recommendation_tags.filter(tag =>
        submittedTags.includes(tag)
      ).length;
      const bMatches = b.recommendation_tags.filter(tag =>
        submittedTags.includes(tag)
      ).length;
  ...
  // Unified cart handler for add/increment/decrement
  ...
  return (
    <div className="quiz-container">
      ...
      <p className="quiz-intro">
        Confused on what dish to order? Don't worry, we've got your back! Answer a few fun questions and we'll recommend dishes you'll love.
      </p>
        ...
        {quizQuestions.map((q) => (
          <div key={q.id} className="quiz-question">
            ...
            <div className="quiz-options">
              {q.options.map((opt) => (
                <label key={opt} className="quiz-option-label">
                  <input
                    type="radio"
                    name={q.id}
                    value={opt}
            ...
            return (
              <div key={item.id} className="quiz-menu-item">
                ...
                <div className="quiz-cart-buttons">
                  {/* Show "Add to Cart" text only when quantity is 0 */}
